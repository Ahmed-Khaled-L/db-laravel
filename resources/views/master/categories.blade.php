...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: "Cairo", "Segoe UI", Tahoma, sans-serif; }
        ...
        .btn-primary { background: #2563eb; color: white; border: none; padding: 10px 16px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 5px; transition: 0.2s; font-size: 14px; text-decoration: none; }
        ...
        .btn-icon { border: none; background: none; cursor: pointer; font-size: 1.1rem; padding: 6px; border-radius: 4px; transition: background 0.2s; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; }
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="display:flex; gap:5px;">
              ...
              <form action="{{ route('categories.destroy', ['id' => $category->id, 'type' => $category->type]) }}"
                    method="POST"
                    onsubmit="return confirm('هل أنت متأكد من حذف هذا البند؟');"
                    style="margin:0;">
                  @csrf
                  @method('DELETE')
