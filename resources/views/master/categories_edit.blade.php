...
<head>
  ...
  <style>
    ...
    .btn-primary { background: #ca8a04; color: white; border: none; padding: 12px; border-radius: 8px; flex: 1; cursor: pointer; font-weight: bold; font-size: 15px; transition: background 0.2s; }
...
<div class="form-page">
  <!-- PUT Route: Passes OLD keys in URL -->
  <form class="card" method="POST" action="{{ route('categories.update', ['id' => $category->id, 'type' => $category->type]) }}">
    @csrf
    @method('PUT')

    <h2>✏️ تعديل بند مخزني</h2>
    ...
    <!-- ID Field (Editable, but warns if changed) -->
    <div class="field">
      <label>رقم البند (ID)</label>
      <input type="number" name="id" required value="{{ old('id', $category->id) }}">
    </div>

    <div class="field">
      <label>اسم البند</label>
      <input type="text" name="cat_name" required value="{{ old('cat_name', $category->cat_name) }}">
    </div>

    <div class="field">
      <label>النوع (جزء من المفتاح الرئيسي)</label>
      <input type="text" name="type" required value="{{ old('type', $category->type) }}" list="typeOptions">
    ...
    <div class="field">
      <label>الجهة / المنشأ</label>
      <input type="text" name="organization" value="{{ old('organization', $category->organization) }}">
    </div>

    <div class="field">
      <label>ملاحظات</label>
      <textarea name="notes" rows="3">{{ old('notes', $category->notes) }}</textarea>
    </div>
